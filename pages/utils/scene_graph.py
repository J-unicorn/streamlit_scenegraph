...
class Model:
    ...
    #       mmcv.imwrite(np.asarray(img), 'bw'+out_file)
        ...
        for i, r in enumerate(relations):
            ...
            if i == num_rel:
                break;
        df=pd.DataFrame(all_rel, columns =['SUBJECT','PREDICATE','OBJECT','VALUE'])
        df.groupby(by=['SUBJECT','PREDICATE','OBJECT'], as_index=False).max()
        return df



    def graph_vis(self,df,src,dst,rel,width=500,height=500):
